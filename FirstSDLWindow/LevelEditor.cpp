...
LevelEditor::LevelEditor(std::string filename) : _filename(filename), _nb_blocks(0), _id_obstacle(ObstacleType::BLOCK), _rotation_obstacle(Direction::UP), _step(0)
{
	...
	for (int x = 0; x < NB_BLOCKS_X_MAX; x++)
	{
		for (int y = 0; y < NB_BLOCKS_Y_MAX; y++)
		{
			if (_grid[x][y][0] != -1)
			{
				...
				renderObstacle(rect_obstacle, (ObstacleType)_grid[x][y][0], (Direction)_grid[x][y][1]);
			}
		}
	}
...
void LevelEditor::renderObstacle(SDL_Rect rect, ObstacleType id_obstacle, Direction rotation)
{
...
{
	_rotation_obstacle = rightRotation(_rotation_obstacle);
}

void LevelEditor::leftRotationObstacle()
{
	_rotation_obstacle = leftRotation(_rotation_obstacle);
}

void LevelEditor::placeObstacle()
{
	if (_id_obstacle != ObstacleType::AIR)
	{
	...
	_grid[_pos_grid_x + _step][_pos_grid_y][0] = (int)_id_obstacle;
	_grid[_pos_grid_x][_pos_grid_y][1] = (int)_rotation_obstacle;
}
